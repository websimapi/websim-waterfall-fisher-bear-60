...
export function createRiggedFish({ scene, score = 0, type = 'classic', opts = {}, addToScene = true }) {
  ...
  for (let i = 0; i < boneCount; i++) {
    ...
    const bellyH = h * 0.25;
    const belly = createVoxel(0, -h * 0.3, segLen * 0.5, w * 0.95, bellyH, segLen * 0.95, bellyMat);
    pivot.add(belly);
  ...
  const swimSpeed = (0.028 + Math.random() * 0.022) * speedMultiplier * sizeSpeedFactor;
...
export function updateRiggedFish(fish, deltaTime = 1) {
  if (!fish) return;
  const ud = fish.userData;

  // Advance forward
  if (ud.thrown) {
    ud.velocity.y += -0.015 * deltaTime; // gravity
    const scaledVelocity = ud.velocity.clone().multiplyScalar(deltaTime);
    fish.position.add(scaledVelocity);
    if (ud.angularVel) {
      fish.rotation.x += ud.angularVel.x * deltaTime;
      fish.rotation.y += ud.angularVel.y * deltaTime;
      fish.rotation.z += ud.angularVel.z * deltaTime;
      ud.angularVel.multiplyScalar(Math.pow(0.992, deltaTime)); // slight damping
    }
    return;
  }
  const scaledVelocity = ud.velocity.clone().multiplyScalar(deltaTime);
  ...
  if (ud.pattern === 'sine') {
    ...
    ud.drift += (Math.random() - 0.5) * 0.01 * deltaTime;
    ud.drift = THREE.MathUtils.clamp(ud.drift, -0.4, 0.4);
    weave = ud.drift;
  }
  const targetX = ud.initialX + weave;
  fish.position.x = THREE.MathUtils.lerp(fish.position.x, targetX, 0.10 * deltaTime);
