...
export function updateWaterfall(waterfallGroup, deltaTime = 1) {
    if (!waterfallGroup) return;
    const cliffEdgeZ = 2.5;
    const cliffTopY = 2;
    waterfallGroup.children.forEach(child => {
        if (child.userData.velocity) {
            const scaledVelocity = child.userData.velocity.clone().multiplyScalar(deltaTime);
            child.position.add(scaledVelocity);
