...
export function updateGame(deltaTime = 1) {
    if (gameState.current === 'PLAYING') {
        ...
        if (bear && bear.position.y > -10) {
            const gravityScaled = gravity.clone().multiplyScalar(deltaTime);
            bear.position.add(gravityScaled);
            bear.rotation.z += 0.05 * deltaTime;
        }
    } else { // IDLE
        gameState.idleAnimTimer += 0.05 * deltaTime;
        if (showcaseBear) {
